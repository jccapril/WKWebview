...
- (UIProgressView *)progressView {
    if (!_progressView) {
        _progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, StatusBarAndNavigationBarHeight, self.view.frame.size.width, 2)];
...
- (WKWebView *)webView {
    if (!_webView) {
        ...
        // 最小字体大小 当将javaScriptEnabled属性设置为NO时，可以看到明显效果
        ...
        // 使用H5的视频播放器在线播放，还是使用原生的播放器全屏播放
        config.allowsInlineMediaPlayback = YES;
        // 设置视频是否需要用户手动播放，WKAudiovisualMediaTypeNone为允许自动播放
        config.mediaTypesRequiringUserActionForPlayback = WKAudiovisualMediaTypeAll;
        // 设置是否允许画中画技术，在特定的设备上有效
        config.allowsPictureInPictureMediaPlayback = YES;
        // 设置请求的user-agent信息中应用程序的名称 iOS9以后可以用
        config.applicationNameForUserAgent = @"jcc";
        
        // 自定义的WKScriptMessageDelegate为了解决循环引用内存不释放的问题
